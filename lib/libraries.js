...
Libraries.prototype.initialize = function(context, callback) {
  ...
  var commandLineLibraries = this.options.libraries || [],
      configLibraries = context.config.attributes.libraries || context.config.attributes.mixins || [],

      allLibraries = commandLineLibraries.concat(configLibraries);

  delete context.config.attributes.mixins;

  // Load local bower paths
  var self = this;
  try {
    // Check if we are in a bower project.
    // This works around https://github.com/bower/bower/pull/534
    // TODO : Remove this conditional once that fix is merged
    var stat = fs.statSync(fu.resolvePath('bower.json'));

    bower.commands.list({paths: true})
        .on('data', function(paths) {
          context.event.emit('debug', 'Loaded bower paths: ' + JSON.stringify(paths));

          self.bowerPaths = paths;
          loadLibraries();
        })
        .on('error', function(error) {
          context.event.emit('log', 'Failed to load bower config: ' + error);

          // Treat this as non-fatal. If we actually need this data it will blow up later
          self.bowerPaths = {};
          loadLibraries();
        });
  } catch (err) {
    context.event.emit('debug', 'Failed to load bower packages: ' + err.stack);
    self.bowerPaths = {};
    loadLibraries();
  }

  function loadLibraries() {
    async.forEachSeries(allLibraries, _.bind(self.load, self, context), callback);
  }
};
...
Libraries.prototype.mapFile = function(resource, library, config) {
  ...
  if (_.isString(resource.bower)) {
    bowerPath = this.bowerPaths[resource.bower];
    if (!bowerPath) {
      // WARN: There is a slight race condtion here, but the hope is that the sync bower call
      //  will complete before this becomes a problem.
      throw new Error('Missing bower package "' + resource.bower + '"');
    }
  }
  ...
  if (src) {
    var librarySrc = bowerPath || library.root || '';
