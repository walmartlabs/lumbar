...
Context.prototype = {
    ...
    function loaded(err, data) {
      ...
      if (err) {
        if (!err.resourceLoadError) {
          ...
          errorWrapper.stack = errorWrapper.message + ' ' + (err.stack || err);
          errorWrapper.source = err;
          errorWrapper.code = err.code;
          errorWrapper.resourceLoadError = true;
          err = errorWrapper;
        }
        callback(err);
        return;
      ...
      context.plugins.fileName(context, function(err, fileName) {
        ...
        writer(function(err, data) {
          data = _.defaults({
              ...
              platformCache: context.platformCache,
          ...
          if (err) {
            fs.unlink(context.fileName, function() { /* NOP To Prevent warning */});
            data.error = err;
          }
  ...
  get description() {
    ...
    if (this.resource) {
      ...
      ret += '_resource:' + resources.source(this.resource);
    }
    return ret;
  },
