...
exports.fileList = function(pathname, extension, callback, dirList, resource) {
  ...
  function handleFiles(watchDir, err, files) {
    var ret = [],
        count = 0,
      ...
      if (count === expected) {
        ...
        if (watchDir) {
          ret = ret.map(function(file) {
            if (file.src) {
              file.watchDir = watchDir;
              return file;
            } else {
              return { src: file, watchDir: watchDir };
            }
          });
  ...
  fs.stat(pathname, function(err, stat) {
    ...
    if (stat.isDirectory()) {
      fs.readdir(pathname, handleFiles.bind(this, dirList || exports.makeRelative(pathname)));
    } else {
