...
exports.fileList = function(pathname, extension, callback, dirList, resource) {
  ...
  exports.stat(pathname, function(err, stat) {
    ...
    exports.readdir(pathname, function(err, files) {
      ...
      callback(undefined, namePasses ? [ resource ? _.defaults({src: pathname, srcDir: srcDir}, resource) : pathname ] : []);
    }
  });
};
...
/*
 * Recursively copy the file or directory identified by the srcPath to the outputPath
 */
exports.copyFile = function(srcFile, outFile, callback) {
  srcFile = exports.resolvePath(srcFile);
  outFile = exports.resolvePath(outFile);

  exports.ensureDirs(outFile, function(err) {
    if (err) {
      return callback(err);
    }
    ...
    function execute() {
      try {
        ...
        if (err.code === 'EMFILE') {
          setTimeout(execute, EMFILE_RETRY);
        } else {
          callback('Could not copy file ' + srcFile + " to " + outFile + ": " + err);
        }
      }
    }
    execute();
