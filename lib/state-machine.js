...
exports.buildPlatform = function(context, callback) {
  ...
  context.platformCache = context.platformCache || {};

  // If in AMD mode then run scripts modes sync before others. This ensures that
  // any scripts the define style or script dependencies will be processed prior
  // to the dependency module's output.
  if (context.config.attributes.amd && _.contains(modes, 'scripts')) {
    modes = _.without(modes, 'scripts');
    exports.buildMode('scripts', context, function(err) {
      if (err) {
        return callback(err);
      }
    ...
    if (context.config.attributes.amd && !context.platformCache.amdConfig) {
      return callback(new Error('Attempting to build mode "' + context.mode + '" without AMD config'));
    }
...
exports.buildMode = function(mode, context, callback) {
  context.event.emit('debug', 'Build mode: ' + mode + ' ' + context.description);

  var modules = context.module ? [context.module] : context.config.moduleList(context.package);

  if (context.config.attributes.amd) {
    var topLevelNames = modules.filter(function(module) {
      return context.config.isTopLevel(module);
    });
    modules = _.difference(modules, topLevelNames);
  }
...
exports.buildModule = function(context, callback) {
  ...
  if (context.config.attributes.amd
      && !context.platformCache.amdConfig
      && !context.config.isTopLevel(module)) {
    return callback(new Error('Attempting to build module "' + context.module + '" without AMD config'));
  }
