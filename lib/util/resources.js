...
var resources = module.exports = {
  ...
  map: function(resource, library, config) {
    ...
    if (_.isString(src)) {
      src = resources.relativePath(src, library, bowerPath);

      var mixinOverride = findOverride(library, src),
          libraryOverride = findOverride(library && library.parent, src),
          override = (mixinOverride || mixinOverride === false) ? mixinOverride : libraryOverride,
  ...
  relativePath: function(src, library, bowerPath) {
    if (src.indexOf('./') === 0) {
      src = src.substring(2);
    }
    src = normalize(src);
