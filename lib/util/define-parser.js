...
function parseDefine(node, allowConfig) {
  ...
  if (deps) {
    expect(deps, 'ArrayExpression', 'Expected dependencies to be ');
...
var workers = {
  ...
  defineRouter: function(node) {
    // Pull the first node off for the routes. Note that this creates an odd structure for
    // explicitly named routers but we're ok with that as that case should be uncommon and the
    // code is much simplier via this route.
    var routesNode = node.arguments[0],
        routes = {};
    expect(routesNode, 'ObjectExpression', 'Expected first argument to be');

    routesNode.properties.forEach(function(prop) {
      if (prop.key.type !== 'Identifier' && prop.key.type !== 'Literal') {
        throw new Error('Expected route to be "Identifier" or "Literal" but found "' + prop.key.type + '"' + errorLocation(prop));
      }
      expect(prop.value, 'Literal', 'Expected route handler to be');

      routes[prop.key.name || prop.key.value] = prop.value.value;
    });
