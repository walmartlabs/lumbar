...
function generateSource(files, includes, styleConfig) {
...
function compile(options, callback) {
  ...
  var source = generateSource(options.files, styleConfig.includes, styleConfig);
  ...
  compiler
    .use(worker.pathScopes(context, module, styleConfig))
