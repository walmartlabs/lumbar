...
var _ = require('underscore'),
    ...
    path = require('path'),
    worker = require('./stylus-worker');

function generateSource(context, files, styleConfig) {
  var includes = (styleConfig.includes || []).concat(files);
  ...
  return declare + includes.map(function(include) {
    var source = include.mixin;
    ...
    if (source) {
      var name = '',
          ...
          container = (source.parent || source).name || '';
      if (source.parent) {
        name = source.name || '';
      }

      return 'push-mixin("' + name + '", "' + container + '", "' + root + '", "' + stylusRoot + '")\n'
          + statement
          + 'pop-mixin()\n';
    } else {
      return statement;
...
function compile(options, callback) {
  ...
  context.fileUtil.ensureDirs(context.fileName, function(err) {
    ...
    worker.exec({
        ...
        useNib: styleConfig.useNib,
        imageOptions: imageOptions,

        filename: options.filename,
        minimize: context.options.minimize,
        source: generateSource(context, options.files, styleConfig),
...
module.exports = {
  ...
  loadMixin: function(context, next, complete) {
    ...
    if (mixinStyles) {
      ...
      if (context.mixins.mergeFiles('includes', styles, mixinStyles, configStyles, context.loadedMixin)) {
        assigned = true;
      }
  ...
  module: function(moduleContext, next, complete) {
    next(function(err) {
      ...
      function mergeResources(start) {
        ...
        generator.stylusFiles = resources.splice(start, rangeEnd - start + 1);
        generator.filename = 'stylus_' + _.map(generator.stylusFiles, function(file) { return file.originalSrc || file.src; }).join(';');
