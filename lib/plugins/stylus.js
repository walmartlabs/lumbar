...
var worker = new ChildPool(__dirname + '/stylus-worker');
...
function generateSource(context, options, styleConfig) {
  ...
  var source = declare + includes.map(function(include) {
    ...
    if (source) {
      ...
      var mixinName = name + '_' + container;

      if (!mixinLUT[mixinName]) {
        var mixinDecl = context.libraries.findDecl(module.mixins, {name: name, container: container}),
...
module.exports = {
  ...
  loadMixin: function(context, next, complete) {
    ...
    if (mixinStyles) {
      ...
      if (context.libraries.mergeFiles('includes', styles, mixinStyles, configStyles, context.loadedMixin)) {
        assigned = true;
      }
