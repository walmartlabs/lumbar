        ...
        async.forEach(_.clone(doc.querySelectorAll(query)), function(el, callback) {
          ...
          context.fileNamesForModule(mode, module, function(err, fileNames) {
            ...
            var content = fileNames.map(function(fileName) {
              // Find the greatest overlap between loadDirName and the path and trim
              var path = dirname(fileName.fileName.path);
              for (var i = 0, len = loadDirName.length; i < len; i++) {
                if (path.indexOf(loadDirName.substring(i)) === 0) {
                  path = path.substring(len-i);
                  break;
                }
              }
              if (path === '.') {
              ...
              return create(loadDirName + path + basename(fileName.fileName.path) + '.' + fileName.fileName.extension);
            });
