...
module.exports = {
  ...
  updateHtmlReferences: function(context, content, callback) {
        ...
        async.forEach(_.clone(doc.querySelectorAll(query)), function(el, callback) {
          ...
          context.fileNamesForModule(mode, module, function(err, fileNames) {
            ...
            var content = fileNames.map(function(fileName) {
              var path = fileName.fileName.path;
              if (path.indexOf(context.platform + '/') === 0) {
                path = path.substring(context.platform.length + 1);
              }
              return create(loadPrefix + path + '.' + fileName.fileName.extension);
            });
    ...
    var doc = jsdom(content, null, { features: { ProcessExternalResources: false, QuerySelector: true } }),
        firstScript = doc.querySelector('script[src^="module:"]');
    if (firstScript) {
      var script = doc.createElement('script');
      script.type = 'text/javascript';
      var loadPrefix = context.config.loadPrefix();
      if (loadPrefix) {
        loadPrefix = loadPrefix.replace(/\/$/, '') + '/' + context.platform + '/';
      } else {
        var platformPath = '/' + context.platform,
            resourcePath = context.fileName.substring(context.outdir.length);
        loadPrefix = path.relative(path.dirname(resourcePath), platformPath);
        if (loadPrefix) {
           loadPrefix += '/';
         }
      }
