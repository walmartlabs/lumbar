...
module.exports = {
  ...
  loadConfig: function(context, next, complete) {
    ...
    _.each(modules, function(module, name) {
      var mixins;
      try {
        mixins = context.mixins.moduleMixins(module);
      } catch (err) {
        errored = true;
      ...
      ['scripts', 'styles', 'static'].forEach(function(field) {
        ...
        _.each(list, function(value, key) {
          if (_.isString(value.mixin)) {
            var mixin = context.mixins.getConfig(value.mixin);
            if (!mixin) {
              errored = true;
              return complete(new Error('Mixin "' + value.mixin + '" not found'));
            }

            list[key] = context.mixins.mapFiles([value], mixin, {})[0];
          }
        });
      });
      ...
      _.each(mixins, function(mixin) {
        ...
        ['scripts', 'styles', 'static', 'routes'].forEach(function(field) {
          mergeValues(module, field, mixin, mixinConfig, context);
        });
      });
    });
...
function mergeValues(module, field, mixin, mixinConfig, context) {
  var value = module[field],
  ...
  if (value === mixinValue) {
    ...
    if (_.isArray(value)) {
      module[field] = context.mixins.mapFiles(value, mixin, mixinConfig);
    } else {
      module[field] = _.clone(value);
    }
