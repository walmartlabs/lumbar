...
module.exports = {
  ...
  loadConfig: function(context, next, complete) {
    var modules = context.config.attributes.modules,
        errored;
    _.each(context.libraries.configs, function(mixin) {
      ...
      _.each(mixin.modules, function(module, key) {
        if (!_.has(modules, key)) {
          ...
          ['scripts', 'styles', 'static', 'routes'].forEach(function(field) {
            ...
            if (_.isArray(value)) {
              module[field] = context.libraries.mapFiles(value, mixin);
            } else if (value) {
...
function mergeValues(module, field, mixin, mixinConfig, context) {
  ...
  if (value === mixinValue) {
    ...
    if (_.isArray(value)) {
      module[field] = context.libraries.mapFiles(value, mixin, mixinConfig);
    } else {
      module[field] = _.clone(value);
    }
    ...
    mixinValue = context.libraries.mapFiles(mixinValue, mixin, mixinConfig);
