...
function ensureModuleTemplates(context) {
  if (!context.configCache.moduleStartTemplate) {
    context.configCache.moduleStartTemplate = handlebars.compile((context.config.attributes.scope && context.config.attributes.scope.startTemplate) || DEFAULT_MODULE_START_TEMPLATE);
  }
  if (!context.configCache.moduleEndTemplate) {
    context.configCache.moduleEndTemplate = handlebars.compile((context.config.attributes.scope && context.config.attributes.scope.endTemplate) || DEFAULT_MODULE_END_TEMPLATE);
  }
}
