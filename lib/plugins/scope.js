...
function ensureModuleTemplates(context, complete) {
  function setTemplates(start, end) {
    context.configCache.moduleStartTemplate = handlebars.compile(start);
    context.configCache.moduleEndTemplate = handlebars.compile(end);
  }
  ...
  if (!context.configCache.moduleStartTemplate) {
    ...
    if (template) {
      if (template.match(/\.handlebars$/)) {
        fu.readFile(template, function(err, data) {
          if (err) {
            complete(err);
          } else {
            var bits = data.toString().split(scopeTemplateDelimiter);
            setTemplates(bits[0], bits[1]);
            complete();
          }
        });
      } else {
        var bits = template.split(scopeTemplateDelimiter);
        setTemplates(bits[0], bits[1]);
        complete();
      }
    } else {
      setTemplates(DEFAULT_MODULE_START_TEMPLATE, DEFAULT_MODULE_END_TEMPLATE);
      complete();
    }
  } else {
    complete();
