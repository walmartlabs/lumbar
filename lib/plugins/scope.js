...
function wrapResources(resources, context) {
  ...
  resources.unshift(generator(moduleStartTemplate({name: context.config.attributes.name, scope: cache.scopeName})));
  resources.push(generator(moduleEndTemplate({name: context.config.attributes.name, scope: cache.scopeName})));
  return resources;
}
