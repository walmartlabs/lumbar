      ...
      function(err, data) {
        ...
        if (!data.inputs) {
          data.inputs = _.chain(resources)
              ...
              .flatten()
              .map(function(resource) { return context.fileUtil.makeRelative(resource.src || resource); })
              .filter(function(resource) { console.log(resource); return _.isString(resource); })
              .value();
          console.log('derived inputs', data.inputs);
        }
