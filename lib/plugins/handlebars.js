        ...
        if (templates.precompile) {
          ...
          if (!options) {
            ...
            if (templates.knownHelpers || options.knownHelpers) {
              var helpersList = _.union(
                  options.knownHelpers || templates.knownHelpers,
                  context.fileCache.knownHelpers);

              options.knownHelpers = helpersList.reduce(
          ...
          try {
            ...
            return callback(new Error('Template "' + name + '": ' + err.message));
          }
        } else {
...
amd.loaders.hbs = {
  resource: function(name, resource, context) {
    ...
    return {src: path.join(templates.root || '', name + '.handlebars')};
  },
  lookup: function(name) {
    return 'Handlebars.templates["' + templateUtil.escapeJsString(name) + '"]';
  }
};
...
amd.loaders.helper = {
  ...
  amdName: function(name) {
    if (/js\/helpers\/(.*)\.js$/.exec(name)) {
      return 'helpers!' + RegExp.$1;
    }
  },
  ...
  output: function(defined, isAppModule, dependencies, context) {
    var helperName = templateUtil.escapeJsString(defined.name.replace(/^.*!/, '')),
        fileId = context.fileCache.amdFileModules[defined.resourceName],
        knownHelpers;

    // Update known helpers list
    if (isAppModule) {
      // Silently dropping on the floor if the knownHelpers feature is not enabled
      var attr = context.config.attributes,
          templates = attr.templates || {};
      knownHelpers = templates.knownHelpers || [];
    } else {
      knownHelpers = context.fileCache.knownHelpers = context.fileCache.knownHelpers || [];
    }
    knownHelpers.push(helperName);

    return [
      amd.registerOutput(defined.resourceName, context),
      amd.generator('('),
      amd.generator(defined.source, defined.resourceName),
      amd.generator(')(' + dependencies + '));\n'),
      amd.generator('Handlebars.registerHelper("' + helperName + '", lwmd[' + fileId + ']);\n')
    ];
  }
};
