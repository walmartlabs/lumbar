...
function ensureTemplateTemplates(context, complete) {
  function setTemplateTemplate() {
    if (!context.configCache.templateTemplate) {
      ...
      if (templateTemplate.match(/\.handlebars$/)) {
        fu.readFile(path.join(context.fileUtil.lookupPath(), templateTemplate), function(err, data) {
          if (err) {
            complete(err);
          } else {
            context.configCache.templateTemplate = handlebars.compile(data.toString());
            setPrecompiledTemplate();
          }
        });
      } else {
        context.configCache.templateTemplate = handlebars.compile(templateTemplate);
        setPrecompiledTemplate();
      }
    } else {
      setPrecompiledTemplate();
  ...
  function setPrecompiledTemplate() {
    if (!context.configCache.precompiledTemplate) {
      ...
      if (precompiledTemplate.match(/\.handlebars$/)) {
        fu.readFile(path.join(context.fileUtil.lookupPath(), precompiledTemplate), function(err, data) {
          if (err) {
            complete(err);
          } else {
            context.configCache.precompiledTemplate = handlebars.compile(data.toString());
            complete();
          }
        });
      } else {
        context.configCache.precompiledTemplate = handlebars.compile(precompiledTemplate);
        complete();
      }
    } else {
      complete();
