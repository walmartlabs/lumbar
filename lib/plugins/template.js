...
function loadTemplate(name, context, callback) {
  ensureTemplateTemplates(context, function(err) {
    ...
    context.fileUtil.readFileArtifact(name, 'template', function(err, cache) {
      ...
      var data = cache.artifact || cache.data.toString(),
      ...
      if (!cache.artifact) {
        ...
        context.fileUtil.setFileArtifact(name, 'template', data);
      }
    ...
    next(function(err, ret) {
      ...
      if (views[resource] && build.filterResource(context.resource, context)) {
        views[resource].forEach(function(template) {
          ret.push({ template: context.mixins.resolvePath(template, context.resource.mixin), name: template });
        });
      }
