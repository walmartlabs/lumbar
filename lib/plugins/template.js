...
function loadTemplate(name, context, callback) {
  if (fu.isDirectory(name)) {
    var ret = [];
    var inputs = fu.filesWithExtension(name, /\.handlebars$/);
    inputs.forEach(function(path) {
      loadTemplate(path, context, function(err, data) {
        if (err) {
          callback(err);
...
module.exports = {
  resourceList: function(context, next) {
    ...
    if (context.mode === 'scripts'
        && views && views[resource] && context.config.filterResource(context.resource, context)) {
