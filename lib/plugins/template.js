...
function ensureTemplateTemplates(context) {
  if (!context.configCache.templateTemplate) {
    context.configCache.templateTemplate = handlebars.compile(context.config.attributes.templateTemplate || DEFAULT_TEMPLATE_TEMPLATE);
  }
  if (!context.configCache.precompiledTemplate) {
    context.configCache.precompiledTemplate = handlebars.compile(context.config.attributes.precompiledTemplate || PRECOMPILED_TEMPLATE);
  }
}
