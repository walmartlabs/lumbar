...
module.exports = {
  ...
  loadMixin: function(context, next, complete) {
    ...
    if (mixinTemplates) {
      ...
      if (context.mixins.mergeHash('auto-include', templates, mixinTemplates, configTemplates)) {
        assigned = true;
      }
    ...
    next(function(err, ret) {
      ...
      function pushTemplates(templates) {
        _.each(templates, function(template) {
          ...
          if (!src || (template.mixin && !template.mixin.attributes)) {
            var templateMixin = template.mixin ? context.mixins.getConfig(template.mixin) : mixin;
            src = context.mixins.resolvePath(template.src || template, templateMixin);
          }

          ret.push({
            src: src,
            name: template.name || template.src || template,
            mixin: template.mixin || mixin,
            template: true
          });
        });
      }
  ...
  remapFile: function(mapping, resource, context) {
    ...
    if (match = mapping.regex.exec(resource)) {
      return _.map(mapping.templates, function(template) {
        ...
        return {name: template, src: context.mixins.resolvePath(template, template.mixin || context.resource.mixin)};
      });
    }
  }
};
