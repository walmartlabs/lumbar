...
module.exports = {
      ...
      next(function(err, resources) {
        ...
        async.map(
            ...
            function(resource, callback) {
              if (build.filterResource(resource, context)) {
  ...
  resource: function(context, next, complete) {
    if (context.resource.amdRegistration) {
      if (!context.fileCache.amdRegistrationOutput) {
        var isAppModule = context.config.isAppModule(context.module),
            registration = 'var lwmd = [];';
        if (isAppModule) {
          registration += 'var wmd = this.__wmd = lwmd;';
        } else {
          registration += 'var wmd = this.__wmd;';
        }

        context.fileCache.amdRegistrationOutput = true;
        complete(undefined, generator(registration));
      } else {
...
module.exports.loaders.view = {
  ...
  output: function(defined, isAppModule, dependencies, context) {
    // TODO : Figure out how we want to propertly name these, this is currently generating "view/nav" rather than just "nav"
    var viewName = templateUtil.escapeJsString(defined.name.replace(/^js\/views\/(.*)\.js$/, '$1')),
  ...
  if (!resource.global && /\bjs\/.*\.js$/.test(resourceName)) {
    ...
    cache = fu.readFileArtifact(resourceName, 'amd', function(err, data) {
        ...
        if (data) {
          cache = {
            defined: _.map(data, function(data) {
              ...
              if (data.define) {
                ret.amd = data.define;
                ret.name = ret.name || resourceName;
          ...
          if (!defines.length) {
            ...
            context.event.emit('debug', 'Dependencies ' + cache.dependencies + ' found for ' + resourceName);
          }
        } else {
          cache = false;
        }
      ...
      async.map(
        ...
        function(err, data) {
          if (data) {
            // Reduce initially so we can track duplicate amdRegistration entries
            data = _.compact(_.flatten(data));
            ...
            if (cache) {
              ...
              // Output once for each root amd resources. The resource plugin will make sure that we
              // only output the source once. This allows us to safely handle race conditions on
              // resource load
              if (!data[0].amdRegistration) {
                data.unshift({amdRegistration: true});
              }
            } else {
...
function loaderFromName(name, context) {
  var loader = module.exports.defaultLoader,
      library;
  if (/^(.*)!(.*)$/.test(name)) {
