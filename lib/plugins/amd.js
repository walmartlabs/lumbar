...
module.exports.loaders.view = {
  ...
  supports: function(data, resource, context) {
    return data.view;
  }
};
...
function parseFile(resource, context, complete) {
  ...
  if (/\.js$/.test(resourceName)) {
    ...
    cache = fu.readFileArtifact(resourceName, 'amd', function(err, data) {
      ...
      context.fileCache.amdFileModules[amdName] = !!cache;
          ...
          if (data) {
            if (cache) {
              ...
              _.each(cache.defined, function(defined) {
                ...
                data.push(module.exports.defaultLoader.output(defined, isAppModule, dependencies, context));
              });
            } else {
...
function loaderFromName(name) {
  ...
  if (/^(.*)!(.*)$/.test(name)) {
    ...
    amdName = name.replace(/js\/(?:(.*?)\/)?(.*)\.js$/, function(match, type, name) {
      if (type) {
