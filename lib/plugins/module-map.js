...
function moduleConfig(context, module, callback) {
  var ret = {},
      commonPrefix,
      preload = module && context.config.module(module).preload,
      depends = module && context.config.module(module).depends;
  if (preload) {
    ret.preload = preload;
  }
  if (depends) {
        ...
        module.exports.buildMap(context, function(err, map, prefix) {
          if (err) {
            ...
            loadModuleMap(map, moduleMap, function(err, data) {
              callback(err, data && {data: data, inputs: [{event: 'module-map'}], generated: true, noSeparator: true, ignoreWarnings: true});
            });
          }
        });
      };
