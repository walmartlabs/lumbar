...
function getModuleMapTemplate() {
  if (!moduleMapTemplate) {
    moduleMapTemplate = handlebars.compile(fs.readFileSync(__dirname + '/module-map.handlebars').toString());
  }
  return moduleMapTemplate;
}

// Force template load before EMFILE may be an issue
getModuleMapTemplate();

function loadModuleMap(map, mapper, callback) {
...
function buildMap(context, options, callback) {
  if (!callback) {
    callback = options;
    options = {};
  }
    ...
    async.forEach(modules, function(module, callback) {
      moduleConfig(context, module, function(err, config, prefix) {
        ...
        if (app.module === module) {
          ...
          if (routes && options.routeCallbacks) {
...
function moduleConfig(context, module, callback) {
  var ret = {},
      commonPrefix,
      moduleConfig = (module && context.config.module(module)) || {},
      preload = moduleConfig.preload,
      depends = moduleConfig.depends,
      serverRender = moduleConfig.serverRender;
  if (serverRender) {
    ret.serverRender = serverRender;
  }
  if (preload) {
    ret.preload = preload;
  }
...
function fileList(context, mode, module, callback) {
  ...
  async.some(modules, function(module, callback) {
      ...
      resourceContext.isModuleMap = true;
  ...
  function(hasResource) {
    ...
    context.fileNamesForModule(mode, module, function(err, configs) {
      ...
      var prefix;
      configs = configs.filter(function(config) { return !config.server; });
  ...
  resource: function(context, next, complete) {
    ...
    if (context.resource['module-map']) {
      var buildModuleMap = function(context, callback) {
        module.exports.buildMap(context, function(err, map, prefix) {
          if (err) {
            callback(err);
          } else {
            var moduleMap = config.attributes.moduleMap || 'module.exports.moduleMap';
            stripPrefix(map, prefix);
            loadModuleMap(map, moduleMap, function(err, data) {
              callback(err, data && {data: data, generated: true, noSeparator: true, ignoreWarnings: true});
            });
          }
        });
      };
