...
exports.init = function(lumbarFile, options) {
    ...
    function buildMode(mode, context, callback) {
      ...
      if (context.fileConfig) {
        processConfig(context.fileConfig);
      } else {
        // wrap our original callback function (which is from bin/lumbar),
        // to call it if there's an error or loop through the configs
        // and invoke processConfig on each one. 
        plugins.generatedFiles(context, function(err, configs) {
          // this function is mapped to the complete parameter to plugins.generatedFiles.
          // we will get called for every plugin that's registered
          // and is a part of the current context's mode and has
          // a function called generated files which in turn 
          // calls complete (us). Otherwise we are guaranteed
          // to be called once after all the plugins have
          // been traversed through. 
          if (err) {
            return callback(err);
          }
          // from there we call processConfig which in turn calls
          // buildModule which then calls config.fileList
          configs.forEach(processConfig);
        });
      }

      function processConfig(fileConfig) {
    ...
    /**
     * This function ...   
     * Gets called from build->processConfig or watch.
     * @name buildModule
     * @function
    ...
    function buildModule(context, callback) {
      ...
      config.fileList(plugins, context, function(err, resources) {
        ...
        function(err, resources) {
          ...
          plugins.module(context, function(err) {
            if (err) {
              return callback(err);
            }
    ...
    function combineResources(context, name, callback) {
      ...
      plugins.file(context, function(err) {
        if (err) {
          callback(err);
        } else {
    ...
    function combineFile(list, context, name, callback) {
      plugins.fileName(context, function(err, fileName) {
        if (err) {
          return callback(err);
        }
        ...
        // FIXME I think the plugins should provide the combiner functionality so lumbar is more generic
        combine.combine(
          list,
          context.fileName,
