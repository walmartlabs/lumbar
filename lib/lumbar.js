...
exports.init = function(lumbarFile, options) {
    var outdir = options.outdir,
        ...
        libraries = new Libraries(options),
        config,
        configCache,
        context;
    ...
    function loadConfig(path, callback) {
      try {
        ...
        context = new Context(options, config, plugins, libraries, event);
        ...
        libraries.initialize(context, function(err) {
          ...
          plugins.loadConfig(context, function(err) {
            ...
            if (options.verbose) {
              console.log('Finalized config', JSON.stringify(context.config.serialize(), undefined, 2));
            }
    ...
    function buildPackages(packageName, modules, callback) {
      if (!callback) {
        callback = modules;
        modules = undefined;
      }
    ...
    return _.extend(event, {
      ...
      build: function(packageName, modules, callback) {
        loadConfig(lumbarFile, function(err) {
          ...
          buildPackages(packageName, modules, callback);
        });
      },
      watch: function(packageName, modules, callback) {
        ...
        loadConfig(lumbarFile, function(err) {
          ...
          var mixinPaths = _.filter(_.pluck(libraries.configs, 'path'), function(path) { return path; });
          ...
          packages.forEach(function(name) {
            buildPackages(name, modules, logError);
          });
        });
      },
      unwatch: function() {
