...
exports.init = function(lumbarFile, options) {
    ...
    return _.extend(event, {
      ...
      moduleMap: function(packageName, callback) {
        if (!callback) {
        ...
        stateMachine.loadConfig(lumbarFile, event, options, function(err, rootContext) {
          ...
          stateMachine.loadPackages(rootContext, packageName, function(err, contexts) {
            ...
            async.forEach(_.keys(contexts), function(packageName, callback) {
                var package = contexts[packageName];
                async.forEach(_.keys(package), function(platformName, callback) {
                    var platform = package[platformName],
                        context = platform[0];

                    rootContext.plugins.get('module-map').buildMap(context, function(err, map) {
                      if (!err) {
