...
exports.create = function(options) {
  ...
  function runPlugins(context, methodName, complete, failOver) {
    ...
    return (function next(complete) {
      ...
      if (plugin) {
        ...
        if ((!context.mode || pluginModes[context.mode].indexOf(plugin) < 0) && modeAll.indexOf(plugin) < 0) {
  ...
  return {
    ...
    loadMixin: function(context, complete) {
      runPlugins(context, 'loadMixin', complete);
    },
    loadConfig: function(context, complete) {
      runPlugins(context, 'loadConfig', complete);
    },
