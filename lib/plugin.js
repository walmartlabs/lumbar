...
const corePlugins = [
    ...
    'scope', 'router', 'template', 'inline-styles', 'coffee-script', 'stylus', 'handlebars',
    'module-map', 'package-config', 'update-externals',
  ...
  function runPlugins(context, methodName, complete, failOver, noMode) {
    ...
    return (function next(complete) {
      ...
      if (plugin) {
        ...
        if (!noMode && (!context.mode || pluginModes[context.mode].indexOf(plugin) < 0) && modeAll.indexOf(plugin) < 0) {
